...
class PositionEncoding(nn.Module):
    def __init__(self, L):
        ...
        self.L = L
        self.channels = 4 * L
        self.freq_bands = 0.5 * np.pi * torch.arange(1, L + 1).float()
    ...
    def forward(self, index_list):
        ...
        x_coords = index_list[:, :, :, 0]
        y_coords = index_list[:, :, :, 1]

        freq = self.freq_bands_buffer.view(1, 1, 1, self.L)
        sin_x = torch.sin(freq * x_coords.unsqueeze(-1))
        cos_x = torch.cos(freq * x_coords.unsqueeze(-1))
        sin_y = torch.sin(freq * y_coords.unsqueeze(-1))
        cos_y = torch.cos(freq * y_coords.unsqueeze(-1))

        pos_enc = torch.cat([sin_x, cos_x, sin_y, cos_y], dim=-1)
...
class NAFNet(nn.Module):
    def __init__(self, img_channel=1, width=32, middle_blk_num=1,
                 ...
                 num_in=4 * 4, num_hidden=64, num_views=9, L=4):
        ...
        # Ending layer
        self.ending = CoordGate(num_in, num_hidden, img_channel, CNN_num_in=chan, CNN_num_out_conv=img_channel, kernel_size=3)
